...
from src.allocation.domain.model import Batch, OrderLine, Product, OutOfStock

today = datetime.now()
tomorrow = today + timedelta(1)
later = today + timedelta(2)
...
def test_returns_allocated_batch_ref():
    ...
    product = Product(sku="MINIMALIST-SPOON", batches=[in_stock_batch, shipment_batch])
...
def test_raises_out_stock_exception_if_cannot_allocate():
    ...
    assert product.event[-1] == events.OutOfStock(sku="SMALL-FORK")
